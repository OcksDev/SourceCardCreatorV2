...
public class RoomLol : MonoBehaviour
{
    ...
    public void PlaceFromCoolRoom(CoolRoom cr, GameObject parent)
    {
        ...
        var gm = Instantiate(sp, CenterSpawn + ((new Vector3(cr.pos.x, cr.pos.y, z) - new Vector3(sz, sz, 0)) * DistanceScaler) + new Vector3((sp.transform.localScale.x/2) - 0.5f, (sp.transform.localScale.y / 2) - 0.5f, 0), parent.transform.rotation, parent.transform);
        SpawnedRooms.Add(gm);
        foreach (var c in cr.comlpetedRooms)
        {
